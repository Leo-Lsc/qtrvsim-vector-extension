...
uint32_t vsetvl_operate(
    RegisterValue avl,
    RegisterValue vtype) {
    uint32_t _a = avl.as_u32();
    uint32_t _b = vtype.as_u32() * 8;
    if (_a >= _b) {
        return _b;
    } else {
        return _a;
...
std::array<RegisterValue, 32> vlw_v_operate( // vlw.v vd, (rs1)
    RegisterValue base,
    RegisterValue offset,
    uint32_t vl) {

    int32_t a = base.as_i32();
    int32_t b = offset.as_i32();
    int32_t address = a + b;
    std::array<RegisterValue, 32> result;
    for (size_t i = 0; i < vl; i++) {
        result[i] = RegisterValue(address + 4 * i);
    }

    return result;
}

std::array<RegisterValue, 32> vsw_v_operate( // vsw.v vs3, (rs1)
    RegisterValue base,
    RegisterValue offset,
    uint32_t vl) {
...
std::array<RegisterValue, 32> vadd_vv_operate(
    std::array<RegisterValue, 32> vs2,
    std::array<RegisterValue, 32> vs1,
    uint32_t vl) {
    
    std::array<RegisterValue, 32> result;
    
    for (size_t i = 0; i < vl; ++i) {
        int32_t a = vs2[i].as_i32();
        int32_t b = vs1[i].as_i32();
        result[i] = RegisterValue(a + b);
    }
    
    return result;
}

std::array<RegisterValue, 32> vadd_vx_operate(
    std::array<RegisterValue, 32> vs2,
    RegisterValue rs1,
    uint32_t vl) {

    int32_t a = rs1.as_i32();
    std::array<RegisterValue, 32> result;
    for (size_t i = 0; i < vl; ++i) {
        int32_t b = vs2[i].as_i32();
        result[i] = RegisterValue(b + a);
    }
    
    return result;
}

std::array<RegisterValue, 32> vadd_vi_operate(
    std::array<RegisterValue, 32> vs2,
    RegisterValue imm,
    uint32_t vl) {
        
    int32_t a = imm.as_i32();
    std::array<RegisterValue, 32> result;
    for (size_t i = 0; i < vl; ++i) {
        int32_t b = vs2[i].as_i32();
        result[i] = RegisterValue(b + a);
...
std::array<RegisterValue, 32> vmul_vv_operate(
    std::array<RegisterValue, 32> vs2,
    std::array<RegisterValue, 32> vs1,
    uint32_t vl) {
...
RegisterValue vred_operate(
    std::array<RegisterValue, 32> vs2,
    uint32_t vl) {
    int32_t sum = 0;
    for (size_t i = 0; i < vl; ++i) {
        // std::cout << vs2[i].as_i32() << std::endl;
        sum += vs2[i].as_i32();
    }

    return RegisterValue(sum);
}
