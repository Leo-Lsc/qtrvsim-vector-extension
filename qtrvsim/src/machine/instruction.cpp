...
static const ArgumentDesc arg_desc_list[] = {
    ...
    // Vector 
    ArgumentDesc('v', 'v', 0, 0x1f, { { { 5, 7 } }, 0 }), // Vector register destination (vd)
    ArgumentDesc('w', 'v', 0, 0x1f, { { { 5, 15 } }, 0 }), // vs1
    ArgumentDesc('x', 'v', 0, 0x1f, { { { 5, 20 } }, 0 }), // vs2
    ArgumentDesc('y', 'v', 0, 0x1f, { { { 5, 7 } }, 0 }), // vs3
    ArgumentDesc('i', 's', 0x10, 0xf, { { { 5, 15 } }, 0 }), // scalar immediate
};
...
static bool fill_argdesbycode() {
    for (const auto &desc : arg_desc_list) {
        // 根据 ArgumentDesc 的名称字符，将其对应的地址存储在 arg_desc_by_code 数组中
...
// Flags for vector instructions
#define FLAGS_VSET_VL (IMF_SUPPORTED | IMF_VECTOR | IMF_REGWRITE | IMF_ALU_REQ_RS | IMF_ALU_REQ_RT) // vsetvl rd, rs1, rs2
#define FLAGS_VLOAD (IMF_SUPPORTED | IMF_VECTOR | IMF_ALUSRC | IMF_V_REGWRITE | IMF_MEMREAD | IMF_MEM | IMF_ALU_REQ_RS)    // vlw.v vd, (rs1)
#define FLAGS_VV_ALU (IMF_SUPPORTED | IMF_VECTOR | IMF_V_REGWRITE | IMF_ALU_REQ_VS2 | IMF_ALU_REQ_VS1) // vadd.vv vd, vs2, vs1
#define FLAGS_VX_ALU (IMF_SUPPORTED | IMF_VECTOR | IMF_V_REGWRITE | IMF_ALU_REQ_VS2 | IMF_ALU_REQ_RS) // vadd.vx vd, vs2, rs1
#define FLAGS_VI_ALU (IMF_SUPPORTED | IMF_VECTOR | IMF_V_REGWRITE | IMF_ALU_REQ_VS2 | IMF_SCALAR) // vadd.vi vd, vs2, imm
#define FLAGS_VSTORE (IMF_SUPPORTED | IMF_VECTOR | IMF_ALUSRC | IMF_MEMWRITE | IMF_MEM | IMF_ALU_REQ_VS3 | IMF_ALU_REQ_RS) // vsw.v vs3, (rs1)
#define FLAGS_VMUL (IMF_SUPPORTED | IMF_VECTOR | IMF_V_REGWRITE | IMF_ALU_REQ_VS2 | IMF_ALU_REQ_VS1 | IMF_V_MUL) // vmul.vv vd, vs2, vs1
#define FLAGS_V_REDUCTION (IMF_SUPPORTED | IMF_VECTOR | IMF_REGWRITE | IMF_ALU_REQ_VS2) // vred rd, vs2
...
// Instruction type for vector instructions
#define IT_VOP Instruction::VOP
#define IT_VL Instruction::VL
#define IT_VS Instruction::VS
#define IT_VRED Instruction::VRED
...
// RV32V
static const struct InstructionMap OP_V_map[] = {
    {"vadd.vv", IT_VOP, NOALU, NOMEM, nullptr, {"v", "w", "x"}, 0x00000057, 0x0000707f, { .flags = FLAGS_VV_ALU }, nullptr}, // vadd.vv vd, vs2, vs1
    IM_UNKNOWN,
    {"vmul.vv", IT_VOP, NOALU, NOMEM, nullptr, {"v", "w", "x"}, 0x00002057, 0x0000707f, { .flags = FLAGS_VMUL }, nullptr}, // vmul.vv vd, vs2, vs1
    {"vadd.vi", IT_VOP, NOALU, NOMEM, nullptr, {"v", "x", "i"}, 0x00003057, 0x0000707f, { .flags = FLAGS_VI_ALU }, nullptr}, // vadd.vi vd, vs2, imm
    {"vadd.vx", IT_VOP, NOALU, NOMEM, nullptr, {"v", "x", "s"}, 0x00004057, 0x0000707f, { .flags = FLAGS_VX_ALU }, nullptr}, // vadd.vx vd, vs2, rs1
    IM_UNKNOWN,
    IM_UNKNOWN,
    {"vsetvl", IT_VOP, NOALU, NOMEM, nullptr, {"d", "s", "t"}, 0x80007057,0xfe00707f, { .flags = FLAGS_VSET_VL }, nullptr}, // vsetvl rd, rs1, rs2
};

// static const struct InstructionMap V_ALU_map[] = {
//     {"vadd.vv", IT_R, NOALU, NOMEM, nullptr, {"d", "s", "t"}, 0x00000057, 0x0000707f, { .flags = FLAGS_VV_ALU }, nullptr}, // vadd.vv
//     IM_UNKNOWN,
//     {"vmul.vv", IT_R, NOALU, NOMEM, nullptr, {"d", "s", "t"}, 0x00002057, 0x0000707f, { .flags = FLAGS_VV_ALU }, nullptr}, // vmul.vv
//     {"vadd.vi", IT_I, NOALU, NOMEM, nullptr, {"d", "s", "j"}, 0x00003057, 0x0000707f, { .flags = FLAGS_VI_ALU }, nullptr}, // vadd.vi
//     {"vadd.vx", IT_R, NOALU, NOMEM, nullptr, {"d", "s", "t"}, 0x00004057, 0x0000707f, { .flags = FLAGS_VX_ALU }, nullptr}, // vadd.vx
//     IM_UNKNOWN,
//     IM_UNKNOWN,
//     IM_UNKNOWN,
// };
...
static const struct InstructionMap I_inst_map[] = {
    ...
    {"vlw.v", IT_VL, NOALU, AC_I32, nullptr, {"v", "o(s)"}, 0x07, 0x7f, { .flags = FLAGS_VLOAD }, nullptr}, // LOAD-FP (vlw.v vd, (rs1)) 
    IM_UNKNOWN, // custom-0
    ...
    {"vsw.v", IT_VS, NOALU, AC_I32, nullptr, {"y", "o(s)"}, 0x27, 0x7f, { .flags = FLAGS_VSTORE }, nullptr}, // STORE-FP (vsw.v vs3, (rs1))
    IM_UNKNOWN, // custom-1
    ...
    IM_UNKNOWN, // NMADD
    IM_UNKNOWN, // OP-FP
    {"op-v", IT_VOP, NOALU, NOMEM, OP_V_map, {}, 0x57, 0x7f, { .subfield = {3, 12} }, nullptr}, // OP-V
...
IMF_SUPPORTED | IMF_REGWRITE | IMF_BRANCH_JALR | IMF_ALUSRC | IMF_ALU_REQ_RS }, inst_aliases_jalr}, // JALR
    {"vred", IT_VRED, NOALU, NOMEM, nullptr, {"d", "x"}, 0x6b, 0x7f, { .flags = FLAGS_V_REDUCTION }, nullptr}, // reserved (vred rd vs2 v0)
...
// 通过另一个 Instruction 对象来创建新对象，并复制指令数据
...
uint8_t Instruction::vd() const {
    return (uint8_t)MASK(5, 7);
}

uint8_t Instruction::vs3() const {
    return (uint8_t)MASK(5, 7);
}
...
int32_t Instruction::immediate() const {
    ...
    // Vector
    case VOP: ret = extend(MASK(5, 15), 5); break; // vadd.vi vd, vs2, imm
    case VL:
    case VS: ret = extend(MASK(12, 20), 12); break; // 暂时把 VL 和 VS 的立即数都当作 12 位立即数处理
    case VRED: break; // vred rd, vs2
    }
    return ret;
}
...
// g: general register
// n: numeric value
// a: Address offset immediate (20bits), encoded in multiples of 2 bytes
// p: B-type immediate for branche
// o: Offset immediate for load/store instructions (12 bits)
// E: 12-bit CSR address

// v: Vector register (vd, vs1, vs2, vs3)
// i: Scalar immediate
// ArgumentDesc('v', 'v', 0, 0x1f, { { { 5, 7 } }, 0 }), // Vector register destination (vd)
// ArgumentDesc('w', 'v', 0, 0x1f, { { { 5, 15 } }, 0 }), // vs1
// ArgumentDesc('x', 'v', 0, 0x1f, { { { 5, 20 } }, 0 }), // vs2
...
// ArgumentDesc('i', 's', 0x10, 0xf, { { { 5, 15 } }, 0 }), // scalar immediate
        ...
        case 'v': val += parse_reg_from_string(field_token, &chars_taken); break;
        case 'p':
