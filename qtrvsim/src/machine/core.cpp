...
DecodeState Core::decode(const FetchInterstage &dt) {
    ...
    RegisterId num_vs1 = (flags & IMF_ALU_REQ_VS1) ? dt.inst.vs1() : 0;
    RegisterId num_vs2 = (flags & IMF_ALU_REQ_VS2) ? dt.inst.vs2() : 0;
    RegisterId num_vd = (flags & IMF_V_REGWRITE) ? dt.inst.vd() : 0;
    RegisterId num_vs3 = (flags & IMF_ALU_REQ_VS3) ? dt.inst.vs3() : 0;
    ...
    std::array<RegisterValue, 32> val_vs1 = regs->read_vr(num_vs1);
    std::array<RegisterValue, 32> val_vs2 = regs->read_vr(num_vs2);
    std::array<RegisterValue, 32> val_vs3 = regs->read_vr(num_vs3);
    const bool v_regwrite = flags & IMF_V_REGWRITE;
    ...
    return { DecodeInternalState {
             ...
             DecodeInterstage { .inst = dt.inst,
                                ...
                                .regwrite = regwrite,
                                .v_regwrite = v_regwrite,
                                .alu_req_rs = bool(flags & IMF_ALU_REQ_RS),
                                .alu_req_rt = bool(flags & IMF_ALU_REQ_RT),
                                .alu_req_vs2 = bool(flags & IMF_ALU_REQ_VS2),
                                .alu_req_vs1 = bool(flags & IMF_ALU_REQ_VS1),
                                .alu_req_vs3 = bool(flags & IMF_ALU_REQ_VS3),
                                .v_mul = bool(flags & IMF_V_MUL),
...
ExecuteState Core::execute(const DecodeInterstage &dt) {
    ...
    RegisterValue alu_fst = RegisterValue(0);  // 默认初始化
    RegisterValue alu_sec = RegisterValue(0);  // 默认初始化
    RegisterValue alu_val = RegisterValue(0);  // 默认初始化
    std::array<RegisterValue, 32> v_alu_fst = {};  // 默认初始化空数组
    std::array<RegisterValue, 32> v_alu_sec = {};  // 默认初始化空数组
    std::array<RegisterValue, 32> v_alu_val = {};  // 默认初始化空数组
    ...
    if (!dt.v_inst) {
        ...
        if (dt.regwrite && dt.alu_req_rs) {  // vsetvl rd, rs1, rs2
            alu_fst = dt.val_rs;
            alu_sec = dt.val_rt;
            vl = vsetvl_operate(alu_fst, alu_sec);
            alu_val = RegisterValue(vl);
            std::cout << "===================================VSETVL===================================" << std::endl;
            std::cout << "Instruction: " << std::endl;
            std::cout << "AVL: " << alu_fst.as_i32() << std::endl;
            std::cout << "VLM: " << alu_sec.as_i32() << std::endl;
            std::cout << "VL: " << vl << std::endl;
            cycle_count += vl;
        } else if (dt.alu_req_vs3) {  // vsw.v vs3, (rs1)
            alu_fst = dt.val_rs;
            alu_sec = dt.immediate_val;
            v_alu_val = vsw_v_operate(alu_fst, alu_sec, vl);
            cycle_count += vl;
        } else if (!dt.v_mul && dt.v_regwrite && dt.alu_req_vs2 && dt.alu_req_vs1) {  // vadd.vv vd, vs2, vs1
            v_alu_fst = dt.val_vs2;
            v_alu_sec = dt.val_vs1;
            v_alu_val = vadd_vv_operate(v_alu_fst, v_alu_sec, vl);
            cycle_count += vl;
        } else if (dt.v_regwrite && dt.alu_req_vs2 && dt.alu_req_rs) {  // vadd.vx vd, vs2, rs1
            v_alu_fst = dt.val_vs2;
            alu_sec = dt.val_rs;
            v_alu_val = vadd_vx_operate(v_alu_fst, alu_sec, vl);
            cycle_count += vl;
        } else if (dt.v_regwrite && dt.alu_req_vs2 && dt.alusrc) {  // vadd.vi vd, vs2, imm
            v_alu_fst = dt.val_vs2;
            alu_sec = dt.immediate_val;
            v_alu_val = vadd_vi_operate(v_alu_fst, alu_sec, vl);
            cycle_count += vl;
        } else if (dt.v_regwrite && dt.alu_req_rs && dt.alusrc) {  // vlw.v vd, (rs1)
            alu_fst = dt.val_rs;
            alu_sec = dt.immediate_val;
            v_alu_val = vlw_v_operate(alu_fst, alu_sec, vl);
            cycle_count += vl + 32;
        } else if (dt.v_mul) {  // vmul.vv vd, vs2, vs1
            v_alu_fst = dt.val_vs2;
            v_alu_sec = dt.val_vs1;
            v_alu_val = vmul_vv_operate(v_alu_fst, v_alu_sec, vl);
            std::cout << "===================================VMUL.VV===================================" << std::endl;
            std::cout << "Instruction: " << std::endl;
            for (size_t i = 0; i < vl; i++) {
                std::cout << v_alu_fst[i].as_i32() << "*" << v_alu_sec[i].as_i32() << "=" << v_alu_val[i].as_i32() << std::endl;
            }
            cycle_count += 4 * vl;
        } else if (dt.regwrite && dt.alu_req_vs2) { // vred rd, vs2
            v_alu_fst = dt.val_vs2;
            alu_val = vred_operate(v_alu_fst, vl);
            std::cout << "===================================VRED===================================" << std::endl;
            std::cout << "Vector Reduction: " << alu_val.as_i32() << std::endl;
            cycle_count++;
        }
    }
    ...
    return { ExecuteInternalState {
                 .alu_src1 = dt.val_rs,
                 .alu_src2 = alu_sec,
                //  .v_alu_src1 = v_alu_fst,
                //  .v_alu_src2 = v_alu_sec,
...
MemoryState Core::memory(const ExecuteInterstage &dt) {
    ...
    std::array<RegisterValue, 32> towrite_v_val = dt.v_alu_val;
    auto mem_addr = Address(get_xlen_from_reg(dt.alu_val));

    std::array<Address, 32> v_mem_addr = {};
    for (size_t i = 0; i < vl; i++) {
        v_mem_addr[i] = Address(get_xlen_from_reg(dt.v_alu_val[i]));
    }

    bool memread = dt.memread;
    bool memwrite = dt.memwrite;
    bool regwrite = dt.regwrite;
    bool v_regwrite = dt.v_regwrite;
        ...
        if (is_special_access(dt.memctl)) {
            ...
            if (memwrite) {
                if (!dt.v_inst){ 
                    ...
                    std::cout << "===================================VSW.V===================================" << std::endl;
                    std::cout << "Instruction: " << std::endl;
                    for (size_t i = 0; i < vl; i++){
                        mem_data->write_ctl(dt.memctl, v_mem_addr[i], dt.val_vs3[i]);
                        std::cout << "Stored to address" << dt.v_alu_val[i].as_i32() << "; Value: " << dt.val_vs3[i].as_i32() << std::endl;
                    }
                }
                cycle_count++;
            }
            if (memread) { 
                if (!dt.v_inst){ 
                    ...
                    std::cout << "===================================VLW.V===================================" << std::endl;
                    std::cout << "Instruction: " << std::endl;
                    for (size_t i = 0; i < vl; i++){
                        towrite_v_val[i] = mem_data->read_ctl(dt.memctl, v_mem_addr[i]);
                        std::cout << "Loaded from address" << dt.v_alu_val[i].as_i32() << "; Value: " << towrite_v_val[i].as_i32() << std::endl;
                    }
                }
                cycle_count++;
            }
...
WritebackState Core::writeback(const MemoryInterstage &dt) {
    ...
    if (dt.v_regwrite) {
        ...
        for (size_t i = 0; i < vl; i++){
            regs->write_vr(dt.num_vd, dt.towrite_v_val[i], i);
            // std::cout << dt.towrite_v_val[i].as_i32() << std::endl;
        }
    }
    cycle_count++;
